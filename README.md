...
本研究では、物語の登場人物やセッションの時間的変化を示すストーリーライン可視化に対して、StoryFlowを基礎にしつつレイアウト生成手法を改良しました。入力データの階層構造を反映するために、セッションや位置の階層を動的関係木として生成し、離散的順序付けと連続二次最適化を組み合わせるハイブリッド手法を提案します。これにより、線の交差や揺れ、揺れ距離、不要な余白を抑えながら階層情報を維持した高速レイアウトを実現します。さらに階層構造を利用したLOD描画により数千本規模のエンティティ線を扱い、追加・削除、ドラッグ、直線化、バンドリングといったリアルタイム操作を支援します。複雑な物語の進化を直感的に追跡できる可視化を目指した研究です。
